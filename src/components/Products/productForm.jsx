...
export default function NewProductForm({ userLogged }) {
    ...
    const handleSave = async () => {
        ...
        if (productId == undefined) {
            response = await productServices.createProduct(body, userLogged.oauthToken);
            productId = response.insertedId;
        } else {
            response = await productServices.modifyProduct(productId, body, userLogged.oauthToken);
        }
